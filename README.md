### Selam 👋

Buraya düzenli bakmıyorum ama repolarım ve [web sitem](https://kehribar.vercel.app) üzerinden ne yaptığımı az çok takip edebilirsiniz.

Çeşitli açık kaynak yazılımların Türkçe çevirisine katkıda bulunuyorum.

---

#### Şu sıralar üzerinde çalıştıklarım

- 🖥️ **[qs-niri](https://github.com/dpentx/qs-niri)** — Quickshell + Niri tabanlı masaüstü ortamım (duvar kağıdı yönetimi, medya kontrolleri, panel bileşenleri)
- 📚 **[eptran](https://github.com/dpentx/eptran)** — epub'ları Türkçeye otomatik çeviren bir pipeline
- 🌐 **[kehribar-web](https://github.com/dpentx/kehribar-web)** — kişisel sitemin Astro tabanlı, çok dilli, "config ledger" temalı yeniden yapımı
- ❄️ Her şeyi NixOS + flakes ile declarative tutmaya çalışıyorum ([manix](https://github.com/dpentx/manix))

---

#### Nasıl çalışıyorum

Projelerimin çoğu vibe coding ile oluştu, ama bunu dizginlemek için Python, Go ve Kotlin öğrenmeyi düşünüyorum. NixOS + flakes ile her şeyi declarative tutmaya bayılıyorum, o kısmı ciddiyim.

![NixOS](https://img.shields.io/badge/NixOS-5277C3?style=flat&logo=nixos&logoColor=white)
![Niri](https://img.shields.io/badge/Niri-Wayland-blueviolet?style=flat)
![Quickshell](https://img.shields.io/badge/Quickshell-QML-41CD52?style=flat&logo=qt&logoColor=white)
![Android](https://img.shields.io/badge/Android-3DDC84?style=flat&logo=android&logoColor=white)

---

#### Hayatım bir şarkı olsaydı, muhtemelen şu olurdu:

<a href="https://music.youtube.com/watch?v=EzeJVk0gDZ4">
  <img src="https://kehribar.vercel.app/api/card" alt="LOLUET — スプートニク"/>
</a>

---
#### Şu an dinlediğim:

<img src="https://kehribar.vercel.app/api/lastfm" alt="last.fm now playing"/>
